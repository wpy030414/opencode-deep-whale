...
import { getAllRoleImagePaths } from './assets-loader.js'
 ...
 */
export function buildImageInjectionScript(): string {
  const rolePaths = getAllRoleImagePaths()
  ...
  for (const { var: varName, src } of ART_FILES) {
    ...
    const uri = imageFileToDataUri(src)
 ...
 */
function buildBootstrapBody(injectJs: string): string {
  return (
    `;(function(){\n` +
    `  document.documentElement.dataset.skin = "active"\n` +
    `})();\n` +
    buildImageInjectionScript() +
    injectJs +
    `\n`
  )
}
 ...
 */
export function buildBootstrap(params: {
  css: string
  injectJs: string
  marker: string
}): string {
